...
# Minimal setup to check if the server starts properly

# Set up environment variables for API keys
os.environ["GOOGLE_API_KEY"] = st.secrets["GOOGLE_API_KEY"]
os.environ["PINECONE_API_KEY"] = st.secrets["PINECONE_API_KEY"]

# Initialize Pinecone and generative AI services
pinecone.init(api_key=os.environ.get("PINECONE_API_KEY"))
genai.configure(api_key=os.environ["GOOGLE_API_KEY"])

# Set up a basic Streamlit app layout
st.title("ADHD Specialist Australia Clinic Chatbot")

# Initial system message
initial_system_message = "Your role is an assistant for an ADHD specialist clinic called \
...
if "context" not in st.session_state:
    st.session_state.context = [HumanMessage(content=initial_system_message)]
    st.write("Initialized context")
...
if query:
    ...
    st.write(f"Appended query to context: {query}")
    ...
    st.write(f"Assistant response: {response_content}")
# import streamlit as st
# import os
# import google.generativeai as genai
# from langchain_google_genai import ChatGoogleGenerativeAI
